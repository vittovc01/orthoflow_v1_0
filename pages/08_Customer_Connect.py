...
def agent_name():
    return clean(st.session_state.get("agente_nome"))
...
def load_data():
    ...
    keep = [c for c in [
        "id", "data_intervento", "created_at", "codice_cliente", "cliente",
        "struttura", "cartella_clinica", "chirurgo", "agente", "linea"
    ] if c in idf.columns]
    ...
    with pd.ExcelWriter(buffer, engine="xlsxwriter") as writer:
        ...
        for sheet_name, df_sheet in [("Customer Connect", summary_df), ("Dettaglio", detail_df)]:
            ...
            for c, col in enumerate(df_sheet.columns):
                ...
                if col in {"Data reintegro", "Data intervento"}:
                    ws.set_column(c, c, 14, date_fmt)
                if col == "Ora registrazione":
                    ws.set_column(c, c, 20, dt_fmt)
            if len(df_sheet.columns) > 0:
                ws.autofilter(0, 0, len(df_sheet), len(df_sheet.columns)-1)
                ws.freeze_panes(1, 0)
        info = wb.add_worksheet("Regola")
        info.write("A1", "Customer Connect")
        info.write("A2", "Cutoff giornaliero")
        info.write("B2", "12:30")
        info.write("A3", "Regola")
        info.write("B3", "Registrazioni entro le 12:30 = reintegro stesso giorno; dopo le 12:30 = reintegro giorno successivo")
        info.write("A4", "Data estrazione")
        info.write("B4", target_date.strftime("%d/%m/%Y"))
    buffer.seek(0)
    return buffer.getvalue()


st.title("🔁 Customer Connect")
st.caption("Estrazione giornaliera reintegri con cutoff automatico alle 12:30.")

try:
    data = load_data()
except Exception as e:
    st.error(f"Errore lettura Customer Connect: {e}")
    st.stop()

if data.empty:
    st.info("Nessun materiale disponibile per Customer Connect.")
    st.stop()

c1, c2, c3 = st.columns([1.3, 1, 2])
target_date = c1.date_input("Data reintegro", value=date.today())
c2.metric("Cutoff", "12:30")
c3.info("Tutto ciò che viene registrato dopo le 12:30 passa automaticamente al reintegro del giorno successivo.")

selected = data[pd.to_datetime(data["Data reintegro"], errors="coerce").dt.date == target_date].copy()

if selected.empty:
    st.warning(f"Nessun reintegro assegnato al {target_date.strftime('%d/%m/%Y')}.")
    st.stop()
...
summary = (selected.groupby("codice", as_index=False)["quantita"].sum()
           .rename(columns={"codice": "Codice", "quantita": "Quantità"})
           .sort_values("Codice"))

detail_cols = [
    ("intervento_id", "Intervento"), ("data_intervento", "Data intervento"),
    ("Ora registrazione", "Ora registrazione"), ("Data reintegro", "Data reintegro"),
    ("Struttura", "Struttura"), ("Cartella clinica", "Cartella clinica"),
    ("codice", "Codice"), ("descrizione", "Descrizione"), ("lotto", "Lotto"),
    ("quantita", "Quantità"), ("linea", "Linea"), ("agente", "Agente"),
]
...
if "Data intervento" in detail.columns:
    detail["Data intervento"] = pd.to_datetime(detail["Data intervento"], errors="coerce")
if "Data reintegro" in detail.columns:
    detail["Data reintegro"] = pd.to_datetime(detail["Data reintegro"], errors="coerce")
if "Ora registrazione" in detail.columns:
    detail["Ora registrazione"] = pd.to_datetime(detail["Ora registrazione"], errors="coerce")
...
st.download_button(
    "⬇️ Scarica Excel Customer Connect",
    data=excel,
    file_name=f"Customer_Connect_{target_date.strftime('%Y%m%d')}.xlsx",
    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
    type="primary",
    use_container_width=True,
)

st.subheader("Customer Connect · Codice + Quantità")
...
with st.expander("Dettaglio interventi inclusi", expanded=False):
    st.dataframe(
        ...
        column_config={
            ...
            "Data reintegro": st.column_config.DateColumn("Data reintegro", format="DD/MM/YYYY"),
            "Ora registrazione": st.column_config.DatetimeColumn("Ora registrazione", format="DD/MM/YYYY HH:mm"),
        },
    )
