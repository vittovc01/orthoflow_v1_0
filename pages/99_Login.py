...
def password_verify(password, salt, expected_hash):
    try:
        digest = hashlib.pbkdf2_hmac(
            'sha256', str(password).encode('utf-8'), str(salt or '').encode('utf-8'), 210_000
        ).hex()
...
def load_user(username):
    try:
        rows = (client().table('utenti_app').select('*')
                .eq('username', str(username).strip()).limit(1).execute().data or [])
...
def audit_login(row, action='LOGIN'):
    try:
        client().table('audit_log').insert({
            'utente': row.get('username', ''), 'ruolo': row.get('ruolo', ''),
            'agente': row.get('agente_nome', '') or '', 'azione': action,
            'tabella': 'utenti_app', 'record_id': str(row.get('id', '')),
...
[data-testid="stToolbar"], [data-testid="stSidebar"], [data-testid="collapsedControl"] { display:none; }
...
if submitted:
    ...
    if valid_db:
        ...
        emergency = {'username': str(username).strip(), 'ruolo':'Admin', 'agente_nome':'', 'id':''}
        audit_login(emergency, 'LOGIN_EMERGENZA')
