...
.ct-card{background:white;border:1px solid rgba(16,38,46,.08);border-radius:18px;padding:18px;min-height:138px;box-shadow:0 8px 24px rgba(16,38,46,.045)}
.ct-badge{display:inline-block;padding:5px 9px;border-radius:999px;background:rgba(23,128,95,.10);color:#14684F;font-weight:800;font-size:.75rem}
...
interventi=get_table('interventi')
righe=get_table('righe_intervento')
giacenze=get_table('giacenze')
anomalie=get_table('anomalie')
movimenti=get_table('movimenti_magazzino')

if role()=='Agente' and agent():
    if not interventi.empty and 'agente' in interventi.columns:
        interventi=interventi[interventi['agente'].astype(str).str.casefold()==agent().casefold()]
    if not righe.empty and 'intervento_id' in righe.columns and not interventi.empty:
        ids=set(interventi['id'].astype(str))
        righe=righe[righe['intervento_id'].astype(str).isin(ids)]

fatt=0.0
if not righe.empty and 'totale' in righe.columns:
    fatt=pd.to_numeric(righe['totale'],errors='coerce').fillna(0).sum()

aperti=len(anomalie) if not anomalie.empty else 0
if not anomalie.empty and 'risolta' in anomalie.columns:
    aperti=len(anomalie[~anomalie['risolta'].fillna(False).astype(bool)])

c1,c2,c3,c4,c5=st.columns(5)
c1.metric('Fatturato',euro(fatt))
c2.metric('Interventi',len(interventi))
c3.metric('Giacenze',len(giacenze) if role()!='Agente' else '—')
c4.metric('Anomalie aperte',aperti)
c5.metric('Movimenti',len(movimenti) if role() in {'Admin','Magazzino'} else '—')

st.markdown('<div class="ct-section">Accessi rapidi</div>',unsafe_allow_html=True)
if role() in {'Admin','Magazzino'}:
    a,b,c,d=st.columns(4)
    a.page_link('pages/01_WMS.py',label='📦 WMS',use_container_width=True)
    ...
    d.page_link('app.py',label='🏥 Gestionale',use_container_width=True)
else:
    a,b=st.columns(2)
    a.page_link('app.py',label='🏥 Gestionale',use_container_width=True)
    b.info('Area logistica nascosta per il profilo Agente.')

if role() in {'Admin','Magazzino'}:
    st.markdown('<div class="ct-section">Logistica in evidenza</div>',unsafe_allow_html=True)
    try:
        sc=pd.DataFrame(sb().table('v_scadenze_ubicazioni').select('*').execute().data or [])
    except Exception:
        sc=pd.DataFrame()
    x1,x2,x3=st.columns(3)
    if not sc.empty:
        x1.metric('Scaduti',len(sc[sc['stato_scadenza']=='SCADUTO']))
        x2.metric('Urgenti ≤30 gg',len(sc[sc['stato_scadenza']=='URGENTE']))
        x3.metric('Attenzione ≤90 gg',len(sc[sc['stato_scadenza']=='ATTENZIONE']))
        ...
        if not priority.empty:
            ...
            st.success('Nessuna scadenza critica nelle ubicazioni registrate.')
    else:
...
if role() in {'Admin','Magazzino'} and not movimenti.empty:
    ...
    st.info('Nessuna attività recente da mostrare.')
